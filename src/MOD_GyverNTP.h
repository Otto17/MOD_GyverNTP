/*
    ...
    GitHub: https://github.com/Otto17/MOD_GyverNTP
	...
	Добавлена функция последней успешной синхронизации в формате "ДД-ММ-ГГ ЧЧ:ММ:СС".
...
public:
    ...
    uint8_t updateNow() {
		...
		//_lastUpdNTP = true;		// Поднимаем флаг
        return readPacket();
    }
	...
	//Получаем дату и время последней успешной синхронизации
	String getFullFormLastUpdate() {
	static String NewData;
	
	if (!_sync) return NewData = F("Not sync");		// Возвращаем до тех пор, пока не пройдёт синхронизация

	if(_lastUpdNTP == true) {		// Если флаг поднят, то формируем дату и время в формате "ДД-ММ-ГГ ЧЧ:ММ:СС" одной строкой
		String strDate;				// Тут храним дату
        strDate.reserve(10);
        if (day() < 10) strDate += '0';
        strDate += day();
        strDate += '-';
		...
		String strTime;				// Тут храним время
        strTime.reserve(8);
        if (hour() < 10) strTime += '0';
        strTime += hour();
        strTime += ':';
        if (minute() < 10) strTime += '0';
        strTime += minute();
        strTime += ':';
        if (second() < 10) strTime += '0';
        strTime += second();
	
		NewData = strDate + ' ' + strTime;		// Складываем строки с датой и временем через пробел
		
		_lastUpdNTP = false;					// Опускаем флаг
	}
	
	return NewData;		// Возвращаем сформированную строку
}
...
private:
	...
	bool _lastUpdNTP = false;		// Флаг для формирования последней успешной синхронизации
